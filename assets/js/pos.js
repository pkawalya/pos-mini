...
let start_date = start.format('YYYY-MM-DD HH:mm:ss');
let end_date = end.format('YYYY-MM-DD HH:mm:ss');
let by_till = 0;
let by_user = 0;
let by_status = 1;
...
$(function () {
  ...
  // Update the start_date and end_date variables when date range changes
  ...
  $(document).ready(function () {
      ...
      products.forEach((product, index) => {
        ...
        product_list +=
          `<tr>
            ...
            <td>${product.name}</td>
            <td>${settings.symbol}${product.cost_price}</td>
    ...
    $("#saveUser").on("submit", function (e) {
      ...
      let userData = {
        ...
        perm_products: $("#perm_products").is(":checked") ? "on" : "off",
        perm_categories: $("#perm_categories").is(":checked") ? "on" : "off", 
        perm_transactions: $("#perm_transactions").is(":checked") ? "on" : "off",
        perm_users: $("#perm_users").is(":checked") ? "on" : "off",
        perm_settings: $("#perm_settings").is(":checked") ? "on" : "off"
      };

      // Validate password match
      if (userData.password !== $("#pass").val()) {
...
function loadTransactions() {
  ...
  // Format dates to include full time range for the day
  let startDate = moment(start_date).startOf('day').format('YYYY-MM-DD HH:mm:ss');
  let endDate = moment(end_date).endOf('day').format('YYYY-MM-DD HH:mm:ss');
  
  let query = `by-date?start=${startDate}&end=${endDate}&user=${by_user}&status=${by_status}&till=${by_till}`;
  ...
  $.get(api + query, function (transactions) {
    if (transactions.length > 0) {
      ...
      transactions.forEach((trans, index) => {
        // Check if transaction date falls within the selected range
        let transDate = moment(trans.date);
        if (transDate.isBetween(startDate, endDate, 'day', '[]')) {
          ...
          if (!isNaN(total)) {
            ...
            // Calculate profit for each item in the transaction
            trans.items.forEach((item) => {
              // Get the product to find its cost price
              let product = allProducts.find(p => p._id === parseInt(item.id));
              let itemCost = product ? (product.cost_price || 0) : 0;
              
              const itemTotal = item.price * item.quantity;
              const totalItemCost = itemCost * item.quantity;
              const itemProfit = itemTotal - totalItemCost;
              
              transactionProfit += itemProfit;
              transactionCost += totalItemCost;
          ...
          transaction_list += `<tr>
            ...
            <td>${trans.till}</td>
            <td>${trans.user}</td>
            <td>${settings.symbol}${parseFloat(transactionProfit).toFixed(2)}</td>
            <td>${trans.paid == "" ? 
                '<button class="btn btn-dark"><i class="fa fa-search-plus"></i></button>' : 
                '<button onClick="$(this).viewTransaction(' + index + ')" class="btn btn-info"><i class="fa fa-search-plus"></i></button>'}</td>
          </tr>`;
        }
      });
      ...
      if (counter > 0) {
        // Update all profit cards
        $("#today_sales").text(settings.symbol + parseFloat(sales).toFixed(2));
        $("#today_cost").text(settings.symbol + parseFloat(totalCost).toFixed(2));
        $("#today_profit").text(settings.symbol + parseFloat(totalProfit).toFixed(2));
        $("#today_orders").text(transact);
        $("#items_sold").text(itemCount);
        
        // Calculate and display profit margin
        const profitMargin = sales > 0 ? ((totalProfit / sales) * 100) : 0;
        ...
        $("#total_profit #counter").text(settings.symbol + parseFloat(totalProfit).toFixed(2));
        $("#total_transactions #counter").text(transact);
        $("#total_items #counter").text(itemCount);
...
function loadSoldProducts() {
  ...
  sold.forEach((item, index) => {
    ...
    let totalSales = item?.qty * parseFloat(item.price);
    let totalCost = item?.qty * (product.length > 0 ? parseFloat(product[0]?.cost_price || 0) : 0);
    let profit = totalSales - totalCost;

    sold_list += `<tr>
      <td>${item?.product}</td>
      <td>${item?.qty}</td>
      <td>${product[0]?.stock == 1 ? product[0]?.quantity : "N/A"}</td>
      <td>${settings.symbol}${totalSales.toFixed(2)}</td>
      <td>${settings.symbol}${profit.toFixed(2)}</td>
      </tr>`;
